...
public class MultiplayerBulletScript : MonoBehaviour {
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (g.tag == "player" || g.tag == "enemy")
        {
            ...
            else if (g.GetComponent<MultiplayerPlayerController>() != false)
            {
                g.GetComponent<MultiplayerPlayerController>().DoDamage(34);
                Quaternion q = transform.rotation;
                GameObject go = (GameObject)Instantiate(bloodSpray, transform.position, q);
                if (rB.velocity.z > 0)
                {
