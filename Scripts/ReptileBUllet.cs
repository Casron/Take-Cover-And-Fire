...
public class ReptileBUllet : MonoBehaviour {
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (g.tag == "player")
        {
            ...
            {
                g.GetComponent<PlayerController>().DoDamage(5);
            ...
            {
                g.GetComponent<AIscript>().DoDamage(5);
                Quaternion q = transform.rotation;
                GameObject go = (GameObject)Instantiate(bloodSpray, transform.position, q);
                if (rB.velocity.z > 0)
                {
        ...
        if (g.tag != "bullet" && g.tag != "superfluous" && g.tag != "enemy")
        {
            if (g.tag == "barricade")
            {
                Quaternion q = transform.rotation;
